...
The calculus to optimize time-steps interacts poorly with time discretization.
Basically we have to approximate a Dirac-delta function,
    and the strategy that's obvious to me doesn't seem to behave with line-searches.

So, I'm not sure if this file is workable. Favor the proof-of-concept instead.
...
W = 40          # NUMBER OF WINDOWS IN EACH PULSE
...
Consider the pulse amplitude Ω(t) on a single qubit.
    ...
    A bit of calculus gives us the partial derivatives ∂Ω(t)/∂A[j] and ∂Ω(t)/∂s[j].
...
This function gives ∂Ω(t)/∂s[j].

Note that, while the final amplitude is A[W],
    the final steptime is s[W-1].
So don't call this function with j==W!

"""
function pulsegradient_steptime(t, j, pulse)
    return Utils.diracdelta(t - pulse.steptimes[j], τ) * (
        pulse.amplitudes[j] - pulse.amplitudes[j+1]
    )
end
...
To calculate the gradients ∂E/∂A[j] and ∂E/∂s[j],
...
function g!(G, x)
    ...
    for (q, pulse) in enumerate(pulses)
        for j in 1:W
            ...
            # SET THE GRADIENTS G[j] ~= ∂E/∂s[j]
            if j == W; continue; end            # s[W] == T ALWAYS, NOT VARIED
            ∂Ω∂x .= map(t -> pulsegradient_steptime(t,j,pulse), t_)
            G[W+j,q] = ∇Ω[:,q] · ∂Ω∂x
        end

        # SET THE GRADIENTS G[j] ~= ∂E/∂ν
        Δν = h * Utils.basisvector(length(x), 2W*q)
        G[2W,q] = (f(x+Δν) - f0) / h
    end
...
# CONFIGURE BOUNDS
lbounds = Matrix{Float64}(undef, 2W, n); ubounds = Matrix{Float64}(undef, 2W, n)
# # #
lbounds[1:W,:] .= -ΔΩ                       # SET MAXIMUM AMPLITUDE
ubounds[1:W,:] .=  ΔΩ
# # #
lbounds[1+W:end-1,:] .= 0                   # SET STEP-TIME CONSTRAINTS
ubounds[1+W:end-1,:] .= T
    #= NOTE I'm not bothering to force step-times to be ordered.
        Because of the way I define the square pulse, I don't think we need to.
        If steptimes cross, they just introduce a phase-flip in that window,
            and permit parts of the window to overlap with others.
        The degrees of freedom don't change, though, so it should still optimize fine.
...
seed!(0)                                # CHANGE SEED AT WILL!
x0 = Matrix{Float64}(undef, 2W, n)
# # #
x0[1:W,:] .= ΔΩ * (2*rand(W,n) .- 1)                        # RANDOMLY SELECTED AMPLITDUES
x0[1+W:end-1,:] .= repeat( range(0,T,W+1)[2:end-1], 1, n)   # EVENLY SPACED STEP-TIMES
