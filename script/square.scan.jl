...
filename = "dat/square.scan.$Hname.$devicename"
...
x0 = Vector{Float64}(undef, (W+1)*n)
x  = Vector{Float64}(undef, (W+1)*n)
...
for sd in sd_
    ...
    Random.seed!(sd)
    global x0 = reshape(x0, W+1, n)
    x0[1:W,:] .= ΔΩ * (2*rand(W,n) .- 1)
    x0[end,:] .= ΔΔ * (2*rand(n) .- 1) .+ device.ω
    x0 = vec(x0)
